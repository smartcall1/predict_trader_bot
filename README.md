...
BNB Chain 기반 예측시장 **Predict.fun**에서 대형 트레이더(고래)의 거래를 실시간 감지하고 **역방향 복사 매매**를 자동으로 수행하는 봇. Polymarket 고래 봇의 모든 로직을 BNB Chain으로 이식한 집대성 버전.
...
7. [역방향 전략 (Contrarian)](#7-역방향-전략-contrarian)
...
**전략 근거**: 예측시장에서 인기 있는 쪽(고가 쪽)은 종종 과대평가된다. 고래가 쏠리는 반대편은 상대적으로 저평가된 엣지가 존재한다. 고승률 고래가 70%에 베팅하면 반대편 30%는 기댓값 양수 구간이 될 수 있다.
...
| whale_manager | 30초 | REST API 폴링, 고래 DB 업데이트 |
...
│   ├── whale_manager.py       # 고래 탐지 — REST 폴링, DB 관리
│   ├── whale_scorer.py        # 고래 스코어링 — ROI/승률/빈도 종합
...
### 4-2. `predict_copy_bot.py` — 메인 봇 (815줄)
...
self.watcher = run_manager()   # 고래 탐지 시작
...
self._seen_market_signals: dict     # 마켓+방향 30분 TTL 시그널 캐시
...
### 4-3. `whale_manager.py` — 고래 탐지 (345줄)

REST API를 30초마다 폴링하여 대형 거래를 탐지하고 고래 DB를 자동 관리한다.
...
db[address] = {
    "address":      "0xfe6d...",
    "name":         "whale_0xfe6d",   # 최초 탐지 시 자동 생성
    "total_trades": 15,
    "wins": 0, "losses": 0,
    "total_pnl":    0.0,
    "total_volume": 12500.0,          # 누적 거래량 (USDT)
    "last_seen":    1710780000,       # 마지막 탐지 시각 (Unix)
    "score":        0.0,
    "trades":       [...최근 50건...] # 최근 50건만 보관
}
```

---

### 4-4. `whale_scorer.py` — 신뢰도 스코어링 (282줄)

고래의 거래 이력을 분석하여 신뢰도 점수(0.0~1.0)를 계산한다.
...
| **빈도점수** | `min(최근30일 거래수 / 50, 1.0)` | 월 50회 기준 |
| **confidence** | `min(정산거래수 / 20, 1.0)` | 소샘플 비례 할인 |
| **recency_penalty** | 4단계 (최근30일 WIN율 기준) | 85%↑:1.0 / 75~85%:0.75 / 65~75%:0.5 / 65%↓:0.25 |
...
### 4-5. `client_wrapper.py` — API 클라이언트 (498줄)
...
**리더보드** (엔드포인트 미확인 상태 — 3개 시도):
```python
["/v1/leaderboard", "/v1/traders/top", "/v1/stats/leaders"]
```
...
3. _seen_market_signals (메인 봇): marketId:side 30분 TTL
```
...
고래 거래 감지
  ...
  DB 있음 + score = 0 + total_volume < $5000 → SKIP (데이터 불충분)
  DB 없음 + size_usdt < MIN_WHALE_SIZE × 2 → SKIP (신규 무명 고래)
      │
      ▼ Filter 5: 동일 마켓 중복 진입 차단
  [A] 같은 marketId 포지션 보유 중 → SKIP
  [B] 같은 marketId+side 30분 내 처리됨 → SKIP
      │
      ▼ 마켓 API 검증
...
```python
고래: BUY Lakers(YES)@0.70
봇:   BUY Heat(NO)@0.30  ← 역방향 (바이너리 마켓에서만 적용)
```

**`_get_opposite_outcome()` 로직**:
1. `token_id` 또는 `outcome_name`으로 고래 outcome 식별
2. `outcomes` 배열에서 나머지 1개 반환 (바이너리 = 정확히 2개)
3. 멀티아웃컴(3개 이상)은 "반대편" 정의 불명확 → SKIP

**역방향 포지션 MIRROR EXIT 특수 처리**:
```python
고래가 SELL 신호 발생 시:
  일반 포지션 → 청산 검토 (이익 상태만)
  역방향 포지션 → 고래 SELL = 우리에게 유리 → CONTRARIAN HOLD
```
...
포지션이 늘어날수록 `bankroll`이 감소하여 `portfolio`가 줄어들고, `bet_size`도 자동으로 축소된다. 별도 켈리 계산 없이 자연스러운 포지션 사이징을 구현한다.

**예시**: 뱅크롤 $3,000, 포지션 없음 → bet_size = $3,000 / 40 = $75
...
**역방향 포지션 현재가 계산**:
```python
# 역방향(No/Down) 포지션: 현재가 = 1 - YES_best_ask
...
```python
resolution.name.upper() in ("YES", "UP", "ABOVE", "OVER")   → WIN  (price=1.0)
resolution.name.upper() in ("NO", "DOWN", "BELOW", "UNDER") → LOSS (price=0.0)
resolution 필드 없음 → current >= 0.95 → WIN, else LOSS (fallback)
```
...
| `== 0` + vol < $5000 | ❌ 차단 (스코어링 데이터 부족) |
| `== 0` + vol >= $5000 | ✅ 임시 허용 (초기 DB 구축 기간) |
...
| ⏹ **Stop** | `.stop_bot` 파일 생성 후 `os._exit(0)` |

**알림 종류**:
- `🚀` 봇 시작
- `🟢` 진입 (마켓명, 고래명, 체결가, 수수료, 슬리피지)
- `✅` WIN / `❌` LOSS
- `🔴` STOP_LOSS (하락률 포함)
...
```jsonl
{"action": "OPEN",  "pos_key": "65291_0xfe6d_1710780000", "price": 0.302, "pnl": 0, "question": "Lakers vs. Heat", "bankroll_after": 2925.0,  "timestamp": "2026-03-18T17:56:00"}
{"action": "WIN",   "pos_key": "65291_0xfe6d_1710780000", "price": 1.0,   "pnl": 173.8, "question": "Lakers vs. Heat", "bankroll_after": 3098.8, "timestamp": "2026-03-19T02:15:00"}
{"action": "STOP_LOSS", "pos_key": "...", "price": 0.18, "pnl": -51.3, ...}
```
...
# 운영 설정
# ───────────────────────────────────
INITIAL_BANKROLL=3000.0                 # 시작 뱅크롤 (USDT)
...
MIN_WHALE_SIZE_USDT=500.0               # 고래 최소 거래액 ($500)
...
| 버그 | 원인 | 수정 |
|------|------|------|
| **정산 마켓 필터 미작동** | `status == "resolved"` 소문자 비교 (API는 대문자 반환) | `_is_resolved()` 헬퍼 `.upper()` 정규화 |
| **백로그 차단 해제** | `_load_state()` 후 `_startup_time = None` 덮어쓰기 | 해당 코드 제거, startup_time 보존 |
| **워밍업 cursor 미저장** | 첫 폴링 후 cursor 미저장 → 동일 50건 재처리 | 워밍업 완료 시 cursor 저장 |

### 필터 및 정확도 개선

| 개선 | 내용 |
|------|------|
| stale trade 방어 | 오더북 vs 고래 가격 25% 이상 이탈 → 차단 |
| 스프레드 필터 | bid-ask 스프레드 15% 초과 → 유동성 부족 차단 |
| MAX_PRICE 슬리피지 방어 | 현재 ask > MAX_PRICE → 슬리피지 후 초과 예방 차단 |
| 0.50~0.60 구간 차단 | 수익성 불명확 구간 필터링 추가 |
| score=0 전략적 허용 | vol >= $5000 미평가 고래만 임시 허용 (DB 구축 초기) |
| MIRROR EXIT 확장 | 어떤 고래의 SELL이든 해당 마켓 포지션 청산 검토 |
| 역방향 HOLD | 역방향 포지션은 고래 SELL 시 오히려 유리 → CONTRARIAN HOLD |
| 손절 None 방어 | 오더북 조회 실패 시 마지막 가격으로 손절 재시도 |
| WIN PnL 정확화 | `shares × 1.0 - size_usdc` (수수료 포함 정확한 계산) |
| Paper SELL 현실화 | 실제 오더북 bid - 슬리피지 - 매도수수료 반영 |
| shares 과대계산 수정 | `(size - fee) / exec_price` (수수료 차감 후 계산) |
| question 캐시 버그 | `_resolve_question()`: 순수 숫자 저장 감지 → API 재조회 |

### config 기준값 변경 이력

| 설정 | 이전 | 현재 | 이유 |
|------|------|------|------|
| `BET_DIVISOR` | 20 (5%) | 40 (2.5%) | 분산 확대, 리스크 감소 |
| `MAX_POSITIONS` | 20 | 30 | 포지션 다양화 |
| `MAX_PRICE` | 0.93 | 0.85 | 정산 임박 마켓 차단 강화 |
| `fee_bps` | 150 (1.5%) | 200 (2%) | 실제 API 기준 보정 |
...
| 핵심 기능 | ✅ 완료 | 7단계 필터, 역방향 전략, 스코어링, 텔레그램 |
| 페이퍼 실행 | ✅ 진행 중 | Termux 백그라운드 ($3,000 PAPER) |
| 라이브 전환 | 🟡 보류 | 2주 페이퍼 데이터 분석 후 결정 |

### 미구현 / 향후 과제

- **고래 스코어 API 검증**: `/v1/traders/{address}/activity` 응답 필드 확인 필요 (현재 대부분 score=0)
- **리더보드 API**: 엔드포인트 공식 확인 후 고승률 고래 능동 발굴
- **WebSocket 거래 이벤트**: 현재 오더북만 지원 — 거래 이벤트 가능 시 폴링 대체
- **성과 분석 스크립트**: trade_history JSONL 분석 자동화
- **멀티아웃컴 지원**: 3개 이상 결과물 마켓 역방향 전략 설계
...
2. **실거래 전환**: `PAPER_TRADING=False` 전환 전 최소 2주 이상 페이퍼 데이터 분석 필수. 승률, ROI, 최대 낙폭(MDD)을 종합적으로 검토한다.
...
*Predict.fun 고래 카피 트레이딩 봇 — Polymarket 봇의 모든 버그 수정 및 전략을 집대성하여 BNB Chain으로 이식*
